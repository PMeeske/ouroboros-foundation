...
public class InMemoryThoughtStore : IThoughtStore
{
    ...
    public Task SaveThoughtsAsync(string sessionId, IEnumerable<PersistedThought> thoughts, CancellationToken ct = default)
    {
        lock (_lock)
        {
            List<PersistedThought> list = _sessions.GetOrAdd(sessionId, _ => new List<PersistedThought>());
    ...
    {
        if (_sessions.TryGetValue(sessionId, out List<PersistedThought>? thoughts))
        {
