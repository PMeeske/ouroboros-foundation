...
namespace Ouroboros.Core.Tracing
{
    public class TracingService : ITracingService
    {
        ...
        public async Task<Result<Activity, string>> StartActivity(string name, Dictionary<string, object>? tags = null)
        {
            ...
            var parentContext = _currentActivity?.Context ?? default(ActivityContext);
            var activity = DistributedTracing.StartActivity(name, tags: tags?.ToDictionary(kv => kv.Key, kv => (object?)kv.Value), parentContext: parentContext);
